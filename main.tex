...
Our main goal in this paper is to model the dependence structure of a conditional copula using regression trees. However, the conditional copula parameter $\theta(x)$ may have a finite range. Therefore, we use a suitable link function $h$ that maps a sum of trees model to the range of $\theta(x)$ in the following way:
...
\end{align}
Here, $q\left( T_k^\ast\mid T_k^{\eta}\right)$ denotes the tree proposal as described by \citet{chipman98BCART}. They suggested four different tree steps given by:
\begin{itemize}
...
The generation of new terminal node values rely on the proposed tree structure that we represent with $q\left(M_k^\ast\mid T_k^\ast, T_k^{\eta}, M_k^{\eta}\right)$. Clearly, we only need a proposal for \textsc{grow} and \textsc{prune} steps to get new candidates, as the dimension does not change for the other two tree steps. Therefore, the proposal for terminal node values is given by:
\begin{itemize}
...
\end{equation}
where $\#(.)$ denotes the cardinality of a set. Lastly, we update $C_k^{\eta+1}$ using the iterative formula discussed by \citet{haario_AMH}. The formulation in \cref{eq:var:adapt} ensures that the proposal variance represents the variance of the mean value at observations at each terminal node, which is equivalent to using a marginal likelihood in traditional BART models. We summarise this algorithm in \cref{alg:ada:prop}.
...
Lastly, once we have sampled a new $(T_k^{\eta+1},M_k^{\eta+1})$, we update the values of $M_k^{\eta+1}$ using an MH step followed by a Metropolis within Gibbs to update $\sigma_{k}^2$ conditional on $(T_k,M_k)$. We summarise one iteration of the RJ-MCMC algorithm in \cref{alg:MCMC}.
...
Let, $n_R, n_C, n$ denote the number of replicates, number of chains and number of observations respectively; and $\overline{\theta}_{ijk}, {\theta}^{(2.5)}_{ijk}, {\theta}^{(97.5)}_{ijk}$ denote the mean, 2.5th quantile and 97.5th quantile of the conditional copula parameter at the $i$-th observation in the $j$-th MCMC chain of the $k$-th replicate. Most importantly, let $\theta_i$ denote the true value of the conditional copula parameter at the $i$-th observation. Then we define the following metrics
...
Note that, one may also want to use a link function on the conditional Kendall's $\tau$ such that
\begin{equation*}
...
Similarly, we can also consider a transformed beta prior \citep{gokhale_prior_cor} for $\mu_j$ if we wish to model the copula parameter with a link function on Kendall's $\tau$.
