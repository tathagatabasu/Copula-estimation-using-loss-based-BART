...
\begin{align}
	...
	\end{split}
\end{align}
So, to sample from the posterior we use a backfitting algorithm proposed by \citet{chipman2010BART} where we sample $(T_k, M_k)$ conditional on the other $m-1$ pairs of $(T_t,M_t)$. To be more precise, let $(T_{-k},M_{-k})\coloneqq (T_{1},M_{1}), \cdots,(T_{k-1},M_{k-1}),(T_{k+1},M_{k+1}),\cdots, (T_{m},M_{m})$. Then in each MCMC iteration, we sample $(T_k, M_k)$ from the following conditional distribution: 
\begin{equation*}
