...
\end{itemize}
Lastly, let $r_{\gamma;\textsc{move}}\left(T_k,M_k;T_k^\ast, M_k^\ast\right)$ denote the acceptance ratio of moving from $(T_k,M_k)$ to $(T_k^\ast,M_k^\ast)$, such that
\begin{equation*}
	\alpha_{\gamma;\textsc{move}}\left(T_k,M_k;T_k^\ast, M_k^\ast\right) = \min\left\{1,r_{\gamma;\textsc{move}}\left(T_k,M_k;T_k^\ast, M_k^\ast\right)\right\},
\end{equation*}
...
\begin{align*}
	&r_{\gamma;\textsc{grow}}\left(T_k,M_k;T_k^\ast, M_k^\ast\right)\\
	&= \frac{
...
\begin{align*}
	&r_{\gamma;\textsc{prune}}\left(T_k,M_k;T_k^\ast, M_k^\ast\right)\\
	&= \frac{
...
\begin{align*}
	r_{\gamma;\textsc{move}}\left(T_k,M_k;T_k^\ast, M_k^\ast\right)= \frac{
...
\begin{lemma}\label{lemm:bounded:ratio}
	Let $\delta>0$, then $\forall$ $(u_{1i},u_{2i})\in [\delta, 1-\delta]^2$,  the acceptance ratios are bounded. That is, there exist values $0\le b_{\gamma, \textsc{move}} \le B_{\gamma, \textsc{move}} \le \infty$ such that
	\begin{equation*}
		\frac{1}{B_{\gamma, \textsc{move}}} \le r_{\gamma, \textsc{move}}\left(T_k,M_k;T_k^\ast, M_k^\ast\right) \le \frac{1}{b_{\gamma, \textsc{move}}}.
	\end{equation*}
\end{lemma}
...
Recall the state space defined by $\mathcal{T}\times\mathcal{M}$. Then we can express this space as finite union of subspaces such that $\mathcal{T}\times\mathcal{M} \coloneq \bigcup_{i=1}^n \mathcal{T}_i \times \mathcal{M}_i$, where $\mathcal{T}_i$ is the space of all trees with $i$ terminal nodes and $\mathcal{M}_i$ is the vector space of terminal node values such that $\mathcal{M}_i \subset \mathbb{R}^i$.
...
\begin{align*}
	&\mathcal{K}_{\gamma;\textsc{prune}}\left(T^{(i)},M^{(i)};T^{(i-1)},M^{(i-1)}\right)\\
	&\ge \alpha_{\gamma;\textsc{prune}}\left(T^{(i)},M^{(i)};T^{(i-1)},M^{(i-1)}\right)q_{\gamma;\textsc{prune}}\left(T^{(i)},M^{(i)};T^{(i-1)},M^{(i-1)}\right)d\left(T^{(i-1)},M^{(i-1)}\right)\\
	&\ge \frac{1}{B_{\gamma;\textsc{prune}}}\frac{1}{\#\left\{\mathcal{PN}(T^{(i)})\right\}}\mathcal{N}\left(\mu_j^{*};\mu_{j},\gamma_j\right)d\left(T^{(i-1)},M^{(i-1)}\right)\\
	&\ge \frac{1}{B_{\gamma;\textsc{prune}}}\frac{1}{\left\lfloor\frac{i}{2}\right\rfloor}\mathcal{N}\left(\mu_j^{*};\mu_{j},\gamma_j\right)d\left(T^{(i-1)},M^{(i-1)}\right)\\
	&\ge \frac{2}{B_{\gamma;\textsc{prune}}}\frac{1}{i}\mathcal{N}\left(\mu_j^{*};\mu_{j},\gamma_j\right)d\mu_j^\ast.
\end{align*}
Therefore there exist $\nu>0$ such that for all $i=2,\cdots,n$
\begin{equation*}
	\mathcal{K}_{\gamma;\textsc{prune}}\left(T^{(i)},M^{(i)};T^{(i-1)},M^{(i-1)}\right)
	\ge \nu\frac{1}{i}\mathcal{N}\left(\mu_j^{*};\mu_{j},\gamma_j\right)d\mu_j^\ast.
\end{equation*}
Let $(T^{(1)},M^{(1)})$ denote the trivial tree. That is tree with only one node. Then for all $i$, we can reach the trivial tree with non-zero probability. Let $\phi$ be a measure defined as $\phi(T^{(i)},M^{(i)}) = \delta_{1}(i)$.
