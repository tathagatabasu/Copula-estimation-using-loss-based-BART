...
MCMC_known_var_debug <- function(n.iter, X, Y, Y.var, mu.prior.mean, mu.prior.var, prior_list,
                           moves.prob = NULL, starting.tree = NULL){
  if(is.null(starting.tree)){
    rt_old <- generate_random_binary_tree_depth_free(1)
    rt_old <- assign_node_idx(rt_old)
  ...
  for(i in 1:n.iter){
    ...
    cat('depth = ', get_depth(rt_old), ', nterm = ', get_num_terminal_nodes(rt_old), '\n')
    cat(move.type, '\n')
    try_tree_step <- function(tree_old){
      tryCatch(tree_step(move.type = move.type, 
                         old_tree = tree_old, 
                         X = X, Y = Y, Y.var = Y.var, 
                         mu.prior.mean = mu.prior.mean, 
                         mu.prior.var = mu.prior.var, 
                         prior_list = prior_list),
               error = \(e){ list(tree = tree_old, 
                                  error = e)})
    }
    new.tree.list <- try_tree_step(rt_old)
    print('outside tree_step')
    if(is.null(new.tree.list$error)){
      cat('no error \n')
      new.tree <- new.tree.list$tree  
    } else{
      cat('error:', new.tree.list$error[[1]], '\n')
      return(new.tree.list$tree)
    }
    ...
    new.tree <- assign_term_node_values_cond(tree_top = new.tree, 
                                             ...
                                             mu.prior.var = mu.prior.var, 
                                             X = X, Y = Y, Y.var = Y.var)
    print('outside assign')
    #cat('depth = ', get_depth(new.tree), ', nterm = ', get_num_terminal_nodes(new.tree), '\n')
    tree_list[[i]] <- new.tree
    matrix.res[i,] <- c(move.type, get_depth(rt_old), get_num_terminal_nodes(rt_old),
                        new.tree.list$prior.ratio, new.tree.list$lik.ratio, 
                        new.tree.list$trans.ratio, new.tree.list$acc.prob, 
